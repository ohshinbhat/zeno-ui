...
export function generateThemeVariables(tokens: DesignTokens): string {
  return [
    ...
    line("--animate-enter", "aicl-enter var(--duration-normal) var(--ease-enter) both"),
    line("--animate-exit", "aicl-exit var(--duration-fast) var(--ease-exit) both"),
    line("--animate-pop", "aicl-pop var(--duration-normal) var(--ease-enter) both"),
    line("--animate-press", "aicl-press var(--duration-fast) var(--ease-standard) both"),
    line("--animate-toast", "aicl-toast var(--duration-normal) var(--ease-enter) both"),
    line("--animate-dialog", "aicl-dialog var(--duration-normal) var(--ease-enter) both"),
    line("--animate-drawer", "aicl-drawer var(--duration-slow) var(--ease-enter) both"),
    line("--animate-skeleton", "aicl-skeleton 1.4s var(--ease-standard) infinite"),
    line("--animate-tab-indicator", "aicl-tab var(--duration-normal) var(--ease-standard) both")
  ].join("\n");
}

export function generateTailwindThemeCss(tokens: DesignTokens): string {
  return `@theme {
${generateThemeVariables(tokens)}
}`.trim();
}

export function generateRuntimeThemeCss(tokens: DesignTokens, scope = ".aicl-theme"): string {
  return `
${scope} {
${generateThemeVariables(tokens)}
...
${generateAnimationCss(tokens, scope)}
`.trim();
}

export function generateExportBundle(tokens: DesignTokens): {
  tailwindThemeCss: string;
  runtimeCss: string;
  json: string;
} {
  return {
    tailwindThemeCss: generateTailwindThemeCss(tokens),
    runtimeCss: generateRuntimeThemeCss(tokens),
