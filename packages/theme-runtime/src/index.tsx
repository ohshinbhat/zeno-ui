...
function normalizeConfig(config: ZenoTokenConfig | ZenoTokenConfigInput): ZenoTokenConfig {
  const result = readZenoTokenConfig(config);
...
function readCachedConfig(key: string): ZenoTokenConfig | null {
  ...
  try {
    ...
    const result = readZenoTokenConfig(parsed);
...
function writeCachedConfig(key: string, config: ZenoTokenConfig): void {
  ...
  try {
    ...
    // Cache failures should never prevent a valid theme from rendering.
  }
}
...
export async function loadZenoTheme(source: ZenoThemeSource): Promise<ZenoThemeLoadResult> {
  if (source.type === "inline") {
    ...
    if (!result.valid) {
      throw new Error(result.issues.join(" ") || "Inline token config is invalid.");
    }
  ...
  if (!result.valid) {
    throw new Error(result.issues.join(" ") || "Remote token config is invalid.");
  }
